...
Prod_1.0.0	               			 
*****************************************************************************/
package com.alfaris.ipsh.subscription.exception.handler;
...
import com.alfaris.ipsh.subscription.exception.RecordCreateException;
import com.alfaris.ipsh.subscription.exception.RecordNotFoundException;
import com.alfaris.ipsh.subscription.exception.RecordUpdateException;
...
//	@Override
	...
	public final ResponseEntity<Object> handleSubscriptionAlreadyExistsException(SubscriptionAlreadyExistsException ex, WebRequest request) {
		List<JSONObject> details = new ArrayList<>();
		ServiceResponse error = new ServiceResponse(ex.getMessage(),
				messageSource.getMessage(ex.getMessage(), null, LocaleContextHolder.getLocale()), details);
		return new ResponseEntity(error, HttpStatus.OK);
	}
	@ExceptionHandler(NoRecordPresentException.class)
	public final ResponseEntity<Object> handleNoRecordPresentException(NoRecordPresentException ex, WebRequest request) {
		List<JSONObject> details = new ArrayList<>();
		ServiceResponse error = new ServiceResponse(ex.getMessage(),
				messageSource.getMessage(ex.getMessage(), null, LocaleContextHolder.getLocale()), details);
		return new ResponseEntity(error, HttpStatus.OK);
