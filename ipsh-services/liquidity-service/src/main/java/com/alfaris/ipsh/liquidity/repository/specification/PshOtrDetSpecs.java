...
public class PshOtrDetSpecs {
	private static Logger logger = LogManager.getLogger(PshOtrDetSpecs.class);

	private PshOtrDetSpecs() {

	}

	public static Specification<PshOtrDet> getRecordsBySearchSpec(String searchParam) {
		return new Specification<PshOtrDet>() {
			@Override
			public Predicate toPredicate(Root<PshOtrDet> root, CriteriaQuery<?> query,
				...
				try {
					...
					if (!StringUtils.isEmpty(searchParam)) {
						...
						int createdDateTo = Integer.parseInt(createdDateToStr); 
 						...
 						if (!StringUtils.isEmpty(createdDateFrom)) {
 							...
 							Predicate startPredicate = criteriaBuilder.greaterThanOrEqualTo(root.<Date>get("creDate"),
									start);
						...
						if (!StringUtils.isEmpty(createdDateTo)) {
							...
							Predicate endPredicate = criteriaBuilder.lessThan(root.<Date>get("creDate"),
									end);
							if (finalPredicate != null)
					...
					query.orderBy(criteriaBuilder.asc(root.get("creDate")));
				} catch (Exception e) {
