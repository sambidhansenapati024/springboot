...
import com.alfaris.ipsh.liquidity.entity.PshOtrDetPK;



@Repository
public interface PshOtrDetRepository
		extends JpaRepository<PshOtrDet, PshOtrDetPK>, JpaSpecificationExecutor<PshOtrDet> {
	
	@Query("SELECT sum(d.transactionAmount) FROM PshOtrDet d WHERE d.valueDate=:todayDate AND d.status=:statusApplied")
	BigDecimal getTotalOutcoming(Date todayDate, String statusApplied);
